...
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  setup(__props) {
    ...
    watch(hasVolume, (newVal, oldVal) => {
      if (!newVal && oldVal) {
        if (volumeItems.value.length > 0 && volumeItems.value[0].price) {
          const firstPrice = Number(volumeItems.value[0].price);
          if (!isNaN(firstPrice)) {
            productPrice.value = firstPrice;
            if (priceInputRef.value) {
              const formattedPrice = firstPrice.toFixed(2).replace(".", ",");
              priceInputRef.value.value = formattedPrice;
            }
          }
        }
        volumeItems.value = [{ volume: "", price: "" }];
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      _push(`<!--[--><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><title>Редактирование товара</title><link rel="icon" href="@/image/IcoL.ico" type="image/x-icon"><div class="GoBackBody">`);
      ...
      if (hasVolume.value) {
        ...
        ssrRenderList(volumeItems.value, (item, index) => {
          _push(`<div class="volume-item"><input${ssrRenderAttr("value", item.volume)}${ssrRenderAttr("placeholder", `Объем варианта ${index + 1}`)} class="custom-input volume-input"><input${ssrRenderAttr("value", item.price)}${ssrRenderAttr("placeholder", `Цена варианта ${index + 1}`)} class="custom-input price-input">`);
          if (volumeItems.value.length > 1) {
      ...
      _push(`<div class="PageVolumeSelection"><select class="custom-select"><option value="" disabled selected>Выберите категорию товара</option><!--[-->`);
      ...
      _push(`<!--]--></select></div><div class="PageButtonProductAdd"><button class="PageAdminBtnProductAdd">Обновить</button><button class="PageAdminBtnProductDelete">Удалить</button></div></div><div class="PageProductDescriptionBlock"><textarea placeholder="Введите описание товара" class="custom-textarea product-description">${ssrInterpolate(productDescription.value)}</textarea></div></div></div></div>`);
      if (showDeleteModal.value) {
        _push(`<div class="modal-overlay"><div class="modal-content"><p>Вы действительно хотите удалить этот продукт?</p><div class="modal-buttons"><button class="modal-button-delete-admin-product">Удалить</button><button class="modal-button-cancel-admin-product">Отменить</button></div></div></div>`);
      } else {
        _push(`<!---->`);
      }
      _push(`<!--]-->`);
...
_sfc_main.setup = (props, ctx) => {
  ...
  (ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add("pages/admin/sAdmin/admin-product/[id].vue");
